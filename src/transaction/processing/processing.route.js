import { Router } from "express";
import processingcontroller from "./processing.controller.js";
import FetchTransactions from "../fetch/fetchTransactions.controller.js";
const fetch = new FetchTransactions();
const process = Router();
const processingController = new processingcontroller();
process.post(
  "/initiateTuitionPayment",
  processingController.initiateTuitionPayment
);
process.post(
  "/updateTuitionPayment",
  processingController.updateTuitionPayment
);
process.get(
  "/getTransactionDetails",
  processingController.getTransactionDetails
);
process.get(
  "/gettransactionsuccess",
  processingController.getSuccessTransaction
);
process.get("/fetchtuition", async (req, res) => {
  try {
    const params = res.locals.user;
    fetch.getTuitionTransactions(params, (results) => {
      if (!results) {
        console.log("error");
      } else {
        res.status(results.code).json(results);
      }
    });
  } catch (err) {
    console.log("errorherreee", err);
    return res.status(500).json({ message: "Internal Server Error" });
  }
});
// process.get("/fetchall", async (req, res) => {
//   try {
//     const params = res.locals.user;
//     fetch.getAllTransactions(params, (results) => {
//       if (!results) {
//         console.log("error");
//       } else {
//         res.status(results.code).json(results);
//       }
//     });
//   } catch (err) {
//     console.log("errorherreee", err);
//     return res.status(500).json({ message: "Internal Server Error" });
//   }
// });
export default process;
